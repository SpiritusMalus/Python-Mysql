...
Таблица `game_genre` предназначена для жанров игр:
...
Таблица `library` общая библиотека видеоигр, где перечислены юзеры и их купленные игры:
1)id = Первичный ключ
2)user_id = внешний ключ, ссылка на users(id), индексирован для ускорения поиска
3)products_id = внешний ключ, ссылка на products(id)
4)game_name = название игры для удобства
5)created_at = дата покупки игры.
Эта таблица заполняется с помощью триггера = user_id, products_id из order_products(user_id, products_id), game_name из products(name), created_at CURRENT_TIMESTAMP по дэфолту
